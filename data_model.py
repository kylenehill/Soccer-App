import pandas as pd
from data_scripts import data
#from data import can_provs, us_states

def get_df():

	# create lists of dictionary values and keys representing US and CAN states
	can_provs_list = list(data.can_provs.values())
	us_states_list = list(data.us_states.values())
	us_states_keys_list = list(data.us_states.keys())
